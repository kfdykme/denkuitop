...
class TreeCardPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // canvas.drawRect(Rect.fromPoints(Offset.zero, mouseOffset), paint);
    // canvas.drawCircle(mouseOffset, 30, paint);
    // canvas.

    // dataTags.forEach((KfToDoTagData element) {
    //     List<TreeCardNode> cnodes = List.from(nodes.where((node) => node.item.tags.contains(element.name)));
    //     for(int x = 0; x < cnodes.length ; x++) {
    //       for(int y = x+1; y < cnodes.length; y++) {
    //         TreeCardNode nx = nodes[x];
    //         TreeCardNode ny = nodes[y];
    //         paint.color = element.darkColor;
    //         paint.strokeWidth = 1;
    //         canvas.drawLine(nx.postion, ny.postion, paint);
    //           // TextPainter(text: TextSpan(
    //           //   text: nx.acc.toString(),
    //           //   style: TextStyle(color: ViewBuilder.RandomColor())
    //           // ),
    //           // textDirection: TextDirection.ltr,)
    //           // ..layout(maxWidth: size.width, minWidth: 0)
    //           // ..paint(canvas, ny.postion + ny.acc);
    //       }
    //     }
    //   });
    // for(int x = 0; x <nodes.length; x++) {
    ...
    //     TreeCardNode nx = nodes[x];
    //     TreeCardNode ny = nodes[y];
    //     // if ((nx.postion - ny.postion).distance < 100) {
    //     //   nx.acc += (nx.postion - ny.postion) / 10;
    //     //   ny.acc += -(nx.postion - ny.postion) / 10;
    //     // }
    //     canvas.drawLine(nx.postion, ny.postion, paint);
    //   }
    // }
    ...
    // if (this.data != null) {
    //   var x = 0.0 + offset.dy;
    //   var y = 10.0 +offset.dx;
    //   var distance = 20;
    //   var startX = x;
    //   var startY = y;
    //   var cardMargin = 8.0;
    //   // this.data.data.forEach((element) {
    //   //   // canvas.drawCircle(Offset(x, x), r, paint);
    //   //   TextPainter(text: TextSpan(
    //   //     text: element.title,
    //   //     style: TextStyle(color: ViewBuilder.RandomColor())
    //   //   ),
    //   //   textDirection: TextDirection.ltr,)
    //   //   ..layout(maxWidth: size.width, minWidth: 0)
    //   //   ..paint(canvas, Offset(r,x));
    //   //   x+= 20;
    //   // });
    //   print("paint ${this.dataTags.length} ${this.data}");
    //   this.dataTags.forEach((KfToDoTagData element) {
    //       startX = x;
    //       startY = y;
    //       TextPainter textPainter = null;
    //       var maxTextWidth = 200.0;
    //       this.data.data.where((ListItemData itemData ) {
    //         return itemData.tags.contains(element.name);
    //       }).forEach((ListItemData itemData) {
    //          textPainter = TextPainter(text: TextSpan(
    //             text: itemData.title,
    //             style: TextStyle(color: element.lightColor2)
    //           ),
    //           textDirection: TextDirection.ltr,);
    //           textPainter.layout(maxWidth: size.width, minWidth: 0);
    //           textPainter.paint(canvas, Offset(y,x));
    //           maxTextWidth =  textPainter.size.width > maxTextWidth ? textPainter.size.width : maxTextWidth;
    //           x += distance;

    //           if (itemPairs[itemData.path] == null) {
    //             itemPairs[itemData.path] = ItemPair();
    //             itemPairs[itemData.path].item = itemData;
    //           }

    //           itemPairs[itemData.path].positions.add(Offset(y, x));
    //       });
    //       paint.color = element.darkColor;
    //       canvas.drawRect(Rect.fromLTWH(y- cardMargin, startX - cardMargin, maxTextWidth + cardMargin, x - startX + cardMargin), paint);
    //       y += distance;
    //       x += distance;
    //   });

    // //   List<ItemPair> list = List.from();
    // //   list.sort(((a, b) {
    // //     return a.positions.length - b.positions.length;
    // //   }));

    //   var off = Offset(size.width/2 + offset.dx, size.height/2 + offset.dy);
    //   var radiusBase = 20.0;

    // pos.add(off);
    // [this.dataTags[0]].forEach((KfToDoTagData element) {
    //   var currentList = List.from(this.data.data.where((ListItemData itemData ) {
    //         return itemData.tags.contains(element.name);
    //       }));

    //   var ori = pos[0];
    //   var first = itemPairs[currentList[0].path];
    //   paint.color = element.darkColor.withAlpha(255);
    //   var range = (120 / first.positions.length) + first.lastRange;
    //   first.lastRange = range;
    //   var radius = radiusBase * first.positions.length;
    //   canvas.drawCircle(pos.first, radius, paint);
    //   // currentList.forEach((ListItemData itemData) {

    //   //       var ori = pos.removeAt(0);

    //   //       var dx = sin(pi*2 * (range/360)) * radius * 2;
    //   //       var dy = cos(pi*2 * (range/360)) * radius * 2;
    //   //       pos.add(Offset(ori.dx + dx, ori.dy + dy));
    //   //     });

    //   for(int x = 1 ; x < currentList.length; x++) {
    //     var cradius = radiusBase * itemPairs[currentList[x].path].positions.length;
    //     radius = radius > cradius ? radius : cradius;
    //   }
    //   for(int x = 1 ; x < currentList.length; x++) {

    //     var currentRange = range * (x/currentList.length) * 4/5;
    //     var cradius = radiusBase * itemPairs[currentList[x].path].positions.length;
    //     var dx = sin(pi*2 * (currentRange/360)) * cradius * 8;
    //     var dy = cos(pi*2 * (currentRange/360)) * cradius * 8;
    //     var targetPos = Offset(ori.dx + dx,  ori.dy + dy);
    //     var textPainter = TextPainter(text: TextSpan(
    //           text: currentList[x].title,
    //           style: TextStyle(color: element.lightColor2)
    //         ),
    //         textDirection: TextDirection.ltr,);
    //         textPainter.layout(maxWidth: size.width, minWidth: 0);
    ...
    //     paint.style = PaintingStyle.stroke;
    //     Path p = Path();
    //     p.moveTo(pos.first.dx, pos.first.dy);
    //     // paint.color = Colors.white;
    //     Offset middlePoint = Offset(( pos.first.dx - (pos.first.dx - targetPos.dx)/2 ),pos.first.dy - (pos.first.dy - targetPos.dy) );
    //     // canvas.drawCircle(middlePoint, 10, paint);
    //     p.quadraticBezierTo(middlePoint.dx, middlePoint.dy ,targetPos.dx, targetPos.dy);
    //     // canvas.drawArc(Rect.fromPoints(pos.first, targetPos), -pi, pi/2, false, paint);
    //     // canvas.drawRect(Rect.fromPoints(pos.first, tarrgetPos), paint);
    //     canvas.drawPath(p, paint);
    //     // canvas.drawLine(pos.first, targetPos, paint);

    //     paint.style = PaintingStyle.fill;
    //     // paint.color = element.r
    //     canvas.drawCircle(targetPos, cradius, paint);
    //     textPainter.paint(canvas, targetPos);
    //   }

    //   radius = radiusBase * first.positions.length;
    //   canvas.drawCircle(pos.first, radius, paint);
    //    var textPainter = TextPainter(text: TextSpan(
    //           text: currentList[0].title,
    //           style: TextStyle(color: element.lightColor2)
    //         ),
    //         textDirection: TextDirection.ltr,);
    //         textPainter.layout(maxWidth: size.width, minWidth: 0);
    //         textPainter.paint(canvas, pos.first);
    // });
    // }
  }
